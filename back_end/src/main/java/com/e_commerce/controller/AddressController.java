...
public class AddressController {
    ...
    public ResponseEntity<ResponseMessage> saveNewUserAddress(@Validated @RequestBody CreateUserAddressForm createUserAddressForm,
        ...
        if (!user.isPresent()) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "OOP! Not found user at id: " + createUserAddressForm.getUserId());
        }
        ...
        if (!provinceCity.isPresent()) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found province/city at id: " + createUserAddressForm.getProvinceCityId());
        }

        Optional<District> district = districtService.findById(createUserAddressForm.getDistrictId());
        if (!district.isPresent()) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found district at id: " + createUserAddressForm.getDistrictId());
        }

        Optional<Ward> ward = wardService.findById(createUserAddressForm.getWardId());
        if (!ward.isPresent()) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found ward at id: " + createUserAddressForm.getWardId());
        }

        if (!provinceCity.get().getDistrict().contains(district.get())) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found district with id = " + createUserAddressForm.getDistrictId() + " in province/city id: " + createUserAddressForm.getProvinceCityId());
        }

        if (!district.get().getWard().contains(ward.get())) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found ward with id = " + createUserAddressForm.getWardId() + " in district id: " + createUserAddressForm.getDistrictId());
        }
        ...
        if (newSetUserAddresses != null) {
            ...
            throw new ApiRequestException(HttpStatus.NOT_ACCEPTABLE, "Can not return because have not have user match with id: " + createUserAddressForm.getUserId());
        }
    }
    ...
    public ResponseEntity<ResponseMessage> updateUserAddress(@RequestBody CreateUserAddressForm createUserAddressForm, @PathVariable Long userAddressId) {
        ...
        if (!userAddress.isPresent()) {
            throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found user address at id: " + userAddressId);
        }

        User user = userAddress.get().getUserInfo().getUser();
        if (!userService.isUserIdEqualUserPrincipalId(user.getId())) {
            throw new ApiRequestException(HttpStatus.NOT_ACCEPTABLE, "Not match user with user address!");
        }
        ...
        if (provinceCityId != null && districtId != null && wardId != null) {
            ...
            if (!provinceCity.isPresent()) {
                throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found province/city at id: " + provinceCityId);
            }

            Optional<District> district = districtService.findById(districtId);
            if (!district.isPresent()) {
                throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found district at id: " + districtId);
            }

            Optional<Ward> ward = wardService.findById(wardId);
            if (!ward.isPresent()) {
                throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found ward at id: " + wardId);
            }

            if (!provinceCity.get().getDistrict().contains(district.get())) {
                throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found district with id = " + districtId + " in province/city id: " + provinceCityId);
            }

            if (!district.get().getWard().contains(ward.get())) {
                throw new ApiRequestException(HttpStatus.NOT_FOUND, "Not found ward with id = " + wardId + " in district id: " + districtId);
            }
